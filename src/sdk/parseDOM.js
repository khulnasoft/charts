...
const findContainerAncestor = element => {
  ...
  while (element) {
    if (element.hasAttribute("data-khulnasoft-wrapper")) return element
    element = element.parentElement
  }
}
...
export default sdk => {
  ...
  const makeNode = element => {
    const isContainer = element.hasAttribute("data-khulnasoft-wrapper")
  ...
  const elements = [...document.querySelectorAll("[data-khulnasoft],[data-khulnasoft-wrapper]")]
